...
public class NumberPickerView extends View{
    ...
    private int mLengthOfList=140;
    private int mOffectForList=1;
    ...
    private boolean valueArbitrarilySet = false;
    ...
    public void setValues(int minValue, int maxValue, String[] newDisplayedValues) {
        if(maxValue - minValue + 1 != newDisplayedValues.length){
            throw new IllegalArgumentException("maxValue - minValue + 1 should be equal to newDisplayedValues.length, now "
                    + "((maxValue - minValue + 1) is " + (maxValue - minValue + 1)
                    + " newDisplayedValues.length is " + newDisplayedValues.length);
        }
    ...
    public void setMaxValue(int maxValue){
        ...
        updateNotWrapYLimit();
        LogUtil.d("the mMaxValue is = " +mMaxValue);
    }
    ...
    public void setValue(int value, boolean fromUser) {
        ...
        setValueArbitrarilySet(fromUser);
    }

    public synchronized boolean isValueArbitrarilySet() {
        return valueArbitrarilySet;
    }

    public synchronized void setValueArbitrarilySet(boolean valueArbitrarilySet) {
        this.valueArbitrarilySet = valueArbitrarilySet;
    }
    ...
    private boolean mStatusOfFanFlag=false;
    // 设置风机列表的点击状态
    public boolean  getStatusOfFanFlag(){
        return mStatusOfFanFlag;
    }

    public void setStautsOfFanFlag(boolean flag){

        mStatusOfFanFlag=flag;
    }
    ...
    private void clickItem(int showCountIndex){
        if(0 <= showCountIndex && showCountIndex < mShowCount) {
            ...
            int deltaIndex = showCountIndex - mShowCount / 2;
            if (deltaIndex == 0) {
                onScrollStateChange(OnScrollListener.SCROLL_STATE_CLICK);
                onScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
                playSoundEffect(SoundEffectConstants.CLICK);
            } else {
                scrollByIndexSmoothly(deltaIndex);
            }
        }else{
            //wrong
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float amount = Math.abs(mCurrDrawGlobalY - lastTotalScrollY);
        if (amount >= mItemHeight) {
            int x = (int)(amount / mItemHeight);
            if (mCurrDrawGlobalY > lastTotalScrollY) {
                lastTotalScrollY += mItemHeight * x;
            } else {
                lastTotalScrollY -= mItemHeight * x;
            }
            if (!isValueArbitrarilySet()) {
                OnItemScroll(mItemHeight, mCurrDrawGlobalY);
            }
        }
    }
    ...
    private void drawContent(Canvas canvas){
        ...
        if(mMaxValue>5){  // 2019年2月14日15:36:54
            mLengthOfList=140;
            mOffectForList=0;
        }else if(mMaxValue==5){  // 风机 列表
            mLengthOfList=4;
            mOffectForList=1;
        }else {               // 灯光 列表
            mLengthOfList=3;
        ...
        for(int i = 0; i < mShowCount + 1; i++){
            ...
            if(i == mShowCount / 2){//this will be picked
                ...
                if (index >mLengthOfList || index-mOffectForList < 0){
                    textSize = getEvaluateSize(fraction, mTextSizeNormal, 100);
                  //  LogUtil.d("the mTextSizeNormal 1 is = "+textSize+" the fraction is = "+fraction+"getOneRecycleSize() is = " +getOneRecycleSize()+"index is = "+index);
                }
                else{
                    textSize = getEvaluateSize(fraction, mTextSizeNormal, mTextSizeSelected);
                  //  LogUtil.d("the mTextSizeNormal 2 is = "+textSize+" the fraction is = "+fraction+"getOneRecycleSize() is = " +getOneRecycleSize()+"index is = "+index);
                }
                if (index >mLengthOfList || index -mOffectForList< 0) textSizeCenterYOffset = getEvaluateSize(fraction, mTextSizeNormalCenterYOffset,30);
                else textSizeCenterYOffset = getEvaluateSize(fraction, mTextSizeNormalCenterYOffset,mTextSizeSelectedCenterYOffset);

            }else if(i == mShowCount / 2 + 1){
                textColor = getEvaluateColor(1 - fraction, mTextColorNormal, mTextColorSelected);
                if (index >mLengthOfList || index < 0) {
                    textSize = getEvaluateSize(1 - fraction, mTextSizeNormal, 100);
                  //  LogUtil.d("the mTextSizeNormal 3 is = "+textSize);
                }
                else {
                    textSize = getEvaluateSize(1 - fraction, mTextSizeNormal, mTextSizeSelected);
                 //   LogUtil.d("the mTextSizeNormal 4 is = "+textSize);
                }
                if (index >mLengthOfList || index < 0) textSizeCenterYOffset = getEvaluateSize(1 - fraction, mTextSizeNormalCenterYOffset,30);
                else textSizeCenterYOffset = getEvaluateSize(1 - fraction, mTextSizeNormalCenterYOffset,mTextSizeSelectedCenterYOffset);
            }else{
            ...
            //    LogUtil.d("the mTextSizeNormal 5 is = "+textSize);
            }
