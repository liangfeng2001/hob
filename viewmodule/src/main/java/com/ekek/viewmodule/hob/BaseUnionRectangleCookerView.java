...
public abstract class BaseUnionRectangleCookerView extends View {
 /*   private static final int HOB_VIEW_WORK_MODE_POWER_OFF = 0;//
    ...
    public BaseUnionRectangleCookerView(Context context) {
        this(context,null);
    ...
    public BaseUnionRectangleCookerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
       ...
       /* mViewRect.set(0, 0, getWidth(), getHeight());
        ...
        //for test
       // workMode = HOB_VIEW_WORK_MODE_PREPARE_TEMP_SENSOR;

        LogUtil.d("Enter:: BaseUnionRectangleCooker ondraw");
        ...
        switch (workMode) {
            ...
            case HOB_VIEW_WORK_MODE_ABNORMAL_NO_PAN: // 无锅
               ...
               /* p = getPowerOffButtonLocation(getPowerOnWithoutPanBitmap().getWidth());
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getPowerOnWithoutPanBitmap().getWidth() +  HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
            ...
            case HOB_VIEW_WORK_MODE_FIRE_GEAR_WITHOUT_TIMER:  // 火力 + 无 定时
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getPowerOnFireGearWithoutTimerBitmap().getWidth() +  HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
                ...
                mViewRect.set(
                        ...
                        (float) (getCookerViewHeight() / 2 + getPowerOnFireGearWithoutTimerBitmap().getHeight() * 0.28));
            ...
            case HOB_VIEW_WORK_MODE_FIRE_GEAR_WITH_TIMER:  // 火力 + 定时
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getPowerOnFireGearWithTimerBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);

                mPaint.setTypeface(fireGearTypeface);
                mPaint.setTextSize(getFireGearWithTimerFontSize());
                mViewRect.set(
                        30,
                        (getCookerViewHeight()/2 - getPowerOnFireGearWithTimerBitmap().getHeight()/2),
                        getPowerOnFireGearWithTimerBitmap().getWidth(),
                        (float) (getCookerViewHeight() * 0.51 ));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
                ...
                mViewRect.set(
                        ...
                        getPowerOnFireGearWithTimerBitmap().getWidth(),
                        (float) (getCookerViewHeight() / 2 + getPowerOnFireGearWithTimerBitmap().getHeight() * 0.47));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
            ...
            case HOB_VIEW_WORK_MODE_TEMP_GEAR_WITHOUT_TIMER: // 温度 + 无 定时
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getTempGearWithoutTimerBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);

                mPaint.setTypeface(tempGearTypeface);
                mPaint.setTextSize(getTempGearWithoutTimerFontSize());
                mViewRect.set(
                        30,
                        getCookerViewHeight() / 2 - getTempGearWithoutTimerBitmap().getHeight() / 2,
                        getTempGearWithoutTimerBitmap().getWidth(),
                        (float) (getCookerViewHeight() / 2 + getTempGearWithoutTimerBitmap().getHeight() * 0.28));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
            ...
            case HOB_VIEW_WORK_MODE_TEMP_GEAR_WITH_TIMER:  // 温度+ 定时
                ...
                mViewRect.set(
                        30,
                        (float) (getCookerViewHeight() / 4 ),
                        getTempGearWithTimerBitmap().getWidth(),
                        (float) (getCookerViewHeight() * 0.29));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
                ...
                mViewRect.set(
                        30,
                        getCookerViewHeight() / 2, getTempGearWithTimerBitmap().getWidth(),
                        (float) (getCookerViewHeight() / 2 + getTempGearWithTimerBitmap().getHeight() * 0.475));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
            ...
            case HOB_VIEW_WORK_MODE_TEMP_INDICATOR_GEAR_WITHOUT_TIMER:  // 无温度 + paly 键
                ...
                if (blinkFlag == BLINK_FLAG_HAVE_NOT_BLINK) {
                    ...
                    canvas.drawBitmap(getPowerOffButtonBitmap(),getTempGearIndicatorWithoutTimerBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);


                    drawTempIndicator(canvas, getTempIndicatorBitmap(), getTempGearIndicatorWithoutTimerBitmap(), 0.25f);
            ...
            case HOB_VIEW_WORK_MODE_TEMP_INDICATOR_GEAR_WITH_TIMER:  // 火力图标 + 定时
                ...
                if (blinkFlag == BLINK_FLAG_HAVE_NOT_BLINK) {
                    ...
                    canvas.drawBitmap(getPowerOffButtonBitmap(),getTempGearIndicatorWithTimerBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);


                    drawTempIndicator(canvas, getTempIndicatorBitmap(), getTempGearIndicatorWithTimerBitmap(), 0.35f);
                    mPaint.setTextAlign(Paint.Align.CENTER);
                    mPaint.setTypeface(fireGearTypeface);
                    mPaint.setTextSize(getTimerFontSize());
                    mViewRect.set(
                    ...
                    if (currentBlinkOptions == BLINK_OPTIONS_ONE) {//ready to cook
                        ...
                        canvas.drawBitmap(getTempGearWithTimerBitmap(),0,(getCookerViewHeight()/2 - getTempGearWithTimerBitmap().getHeight()/2),null);
                        canvas.drawBitmap(getPowerOffButtonBitmap(),getTempGearWithTimerBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
            ...
            case HOB_VIEW_WORK_MODE_PREPARE_TEMP_SENSOR:  // 精确控温： 温度 + play 键
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getTempPrepareSensorBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
                ...
                mViewRect.set(
                        40,
                        (float) (getCookerViewHeight() / 2 - getTempPrepareSensorBitmap().getHeight() * 0.85),//0.4
                        getTempPrepareSensorBitmap().getWidth(),
                        (float) (getCookerViewHeight() * 1.13));
                fontMetrics = mPaint.getFontMetrics();
                top = fontMetrics.top;//为基线到字体上边框的距离,即上图中的top
                bottom = fontMetrics.bottom;//为基线到字体下边框的距离,即上图中的bottom
                baseLineY = (int) (mViewRect.centerY() - top/2 - bottom/2) + 0;//基线中间点的y轴计算公式
                ...
                mViewRect.set(
                        ...
                        (float) (getCookerViewHeight() / 2 - getTempPrepareSensorBitmap().getHeight() * 0.4),
                        getTempPrepareSensorBitmap().getWidth(),
            ...
            case HOB_VIEW_WORK_MODE_TEMP_SENSOR_READY:  // 蓝牙配对后，ready to cook
                ...
                if (blinkFlag == BLINK_FLAG_HAVE_NOT_BLINK) {
                    ...
                    canvas.drawBitmap(getPowerOffButtonBitmap(),getTempSensorReadyBitmap().getWidth() + + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
                    ...
                    if (currentBlinkOptions == BLINK_OPTIONS_ONE) {
                        ...
                        canvas.drawBitmap(getPowerOffButtonBitmap(),getTempSensorReadyValueBitmap().getWidth() + + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
            ...
            case HOB_VIEW_WORK_MODE_TEMP_WORK_FINISH_WAIT_USER_CONFIRM:  // +10 min    keep warm
                ...
                canvas.drawBitmap(getPowerOffButtonBitmap(),getTempWorkFinishLightBitmap().getWidth() +  HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
            ...
            case HOB_VIEW_WORK_MODE_TEMP_GEAR_WITH_TIMER_AND_RECIPES_FIRST:  // 温度+定时　　＋　图片  601
                ...
                if (blinkFlag == BLINK_FLAG_HAVE_NOT_BLINK) {
                    ...
                    canvas.drawBitmap(getPowerOffButtonBitmap(),getRecipesBgBitmap().getWidth() + HOB_RETANGLE_VIEW_GAP_COOKER_POWER_OFF_BUTTON ,20,null);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        LogUtil.d("Enter:: base union onTouchEvent----event--->" + event.getAction());
    ...
    protected void processClickEvent(MotionEvent event) {
        LogUtil.d("Enter:: base union click event");
        ...
        switch (workMode) {
            ...
            case HOB_VIEW_WORK_MODE_ABNORMAL_NO_PAN:
                ...
                if (touchUpX > getPowerOnFireGearWithoutTimerBitmap().getWidth() * 0.7 && touchUpY < y1 * 0.3) {
            ...
            case HOB_VIEW_WORK_MODE_FIRE_GEAR_WITHOUT_TIMER:
                ...
                if (touchUpX > getPowerOnFireGearWithoutTimerBitmap().getWidth() * 0.7 && touchUpY < y1 * 0.3) {
                    ...
                    recycle();
                    mListener.onCookerPowerOff();
                }else if(touchUpX < getPowerOnFireGearWithoutTimerBitmap().getWidth() && touchUpY < getCookerViewHeight() * 0.8){
                    playSoundEffect(SoundEffectConstants.CLICK);
                    mListener.onSetGear();
                }else if (touchUpX < getPowerOnFireGearWithoutTimerBitmap().getWidth() && touchUpY > getCookerViewHeight() * 0.8) {
                    playSoundEffect(SoundEffectConstants.CLICK);
                ...
                case HOB_VIEW_WORK_MODE_FIRE_GEAR_WITH_TIMER:
                    ...
                    if (touchUpX > getPowerOnFireGearWithoutTimerBitmap().getWidth() * 0.7 && touchUpY < y1 * 0.3) {
                        ...
                        recycle();
                        mListener.onCookerPowerOff();
                    }else if(touchUpX < getPowerOnFireGearWithTimerBitmap().getWidth() && touchUpY < getCookerViewHeight() * 0.5){
                        playSoundEffect(SoundEffectConstants.CLICK);
                        mListener.onSetGear();
                    }else if (touchUpX < getPowerOnFireGearWithTimerBitmap().getWidth() && touchUpY > getCookerViewHeight() * 0.5) {
                        playSoundEffect(SoundEffectConstants.CLICK);
